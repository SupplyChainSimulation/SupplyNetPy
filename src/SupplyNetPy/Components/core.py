...
class Statistics(InfoMixin):
    ...
    def __init__(self, node:object, periodic_update:bool=False, period:float=1):
        ...
        self._stats_keys = ["demand_placed", "demand_received", "fulfillment_received", "shortage", "backorder", "inventory_level", "invetory_carry_cost", "inventory_spend_cost", "transportation_cost", "node_total_cost", "revenue", "profit"]
        ...
        self.demand_placed = {} # demand placed by this node
        self.fulfillment_received = {} # fulfillment received by this node
        self.demand_received = {} # demand received by this node (demand at this node)
        self.demand_fulfilled = {} # demand fulfilled by this node (demand that was served by this node)
        self.orders_shortage = {} # shortage of products at this node 
        self.backorder = {} # any backorders or pending orders at this node
        self.inventory_level = 0 # current inventory level at this node
        self.invetory_carry_cost = 0 # inventory carrying cost at this node
        self.inventory_spend = {} # inventory replenishment cost at this node
        self.transportation_cost = 0 # transportation cost at this node
        self.node_total_cost = 0 # total cost at this node
    ...
    def update_stats(self,**kwargs):
        ...
        for key, value in kwargs.items():
            if hasattr(self, key):
                ...
                if isinstance(attr, dict): # value = [key, value]
                    if value[0] not in attr:
                        attr[value[0]] = value[1]
                    elif isinstance(value, list):
                        attr[value[0]][0] += value[1][0]
                        attr[value[0]][1] += value[1][1]
                    else:
                        attr[value[0]] += value[1]
                    setattr(self, key, attr) # update the attribute with the new value
                    print("value updated", attr)
        ...
        self.node_total_cost = self.invetory_carry_cost + sum([x for x in self.inventory_spend.values()]) + self.transportation_cost
        self.revenue = self.total_products_sold * self.node.sell_price if hasattr(self.node, 'sell_price') else 0
        self.profit = self.revenue - self.node_total_cost
...
class SSReplenishment(InventoryReplenishment, NamedEntity):
    ...
    Attributes:
        ...
        first_review_delay (float): delay before the first review
        ...
        while True: # run the replenishment process indefinitely
            ...
            if (self.node.inventory.inventory.level <= s) and (not self.node.ongoing_order):
                ...
                if supplier.source.inventory.inventory.level < order_quantity:  # check if the supplier is able to fulfill the order, record shortage
                    self.node.logger.logger.info(f"{self.env.now:.4f}:{self.node.ID}: Product not available at {supplier.source.ID}. Waiting for {order_quantity} units.")
        ...
        while True:
            ...
            if(self.node.inventory.inventory.level <= R) and (not self.node.ongoing_order):  # check if inventory level is below reorder point
                ...
                if supplier.source.inventory.inventory.level < Q:  # check if the supplier is able to fulfill the order, record shortage
                    self.node.logger.logger.info(f"{self.env.now:.4f}:{self.node.ID}: Product not available at {supplier.source.ID}. Waiting for {Q} units.")
                    if supplier.source.ID not in self.node.orders_shortage:
                        self.node.orders_shortage[supplier.source.ID] = [0, 0]
                    self.node.orders_shortage[supplier.source.ID][0] += 1 # number of orders short
...
class PeriodicReplenishment(InventoryReplenishment):
    ...
    def __init__(self, env, node, params):
        ...
        self.first_review_delay = params.get('first_review_delay', 0)
        self._info_keys.extend("name", "first_review_delay")
...
class Node(NamedEntity, InfoMixin):
    ...
    Parameters:
        ...
        supplier_selection_policy (str): supplier selection policy (availability, cheapest, fastest, fixed)
        supplier_selection_mode (str): supplier selection mode (dynamic, fixed)
    ...
    def __init__(self, env: simpy.Environment, 
        ...
        self.transportation_cost = 0 # total transportation costs
...
class Supplier(Node):
    ...
    def __init__(self, 
        ...
        self.stats = Statistics(self,periodic_update=True,period=1)
    ...
    def behavior(self):
        ...
        while True:
            if(self.inventory.inventory.level < self.inventory.inventory.capacity): # check if the inventory is not full
                ...
                self.stats.update_stats(total_raw_materials_mined=mined_quantity)
...
class InventoryNode(Node):
    ...
    def __init__(self,
                 ...
                 supplier_selection_policy: SupplierSelectionPolicy = SelectAvailable,
    ...
    def process_order(self, supplier, reorder_quantity):
        ...
        if(supplier.source.node_status == "active"):
            ...
            if(reorder_quantity <= 0): # if the reorder quantity is less than or equal to 0, do not place the order
                ...
                self.logger.logger.info(f"{self.env.now:.4f}:{self.ID}:Inventory replenished. reorder_quantity={reorder_quantity}, Inventory full, order discarded.")
                return
...
class Manufacturer(Node):
    ...
    def __init__(self,
        ...
        if(self.env == None):
            self.node.logger.logger.error("Simulation environment not provided.")
    ...
    def behavior(self):
        ...
        if(len(self.suppliers)==0):
            self.node.logger.logger.warning("No suppliers connected to the manufacturer.")

...
def validate_positive(name: str, value):
    """
    Check if the value is positive (>0) and raise ValueError if not.
...
def validate_non_negative(name: str, value):
    """
    Check if the value is non-negative (>=0) and raise ValueError if not.
...
class Statistics(InfoMixin):
    """
    Statistics class represents an object for a node to store and keep updated all node related statistics during the simulation.

    Parameters: 
        node (object): node to which this statistics object belongs
        periodic_update (bool): whether to update statistics periodically
        period (float): period for periodic update of statistics

    Attributes:
        node (object): node to which this statistics object belongs
        name (str): name of the statistics object
        demand_placed (list): total orders placed and total quantity demanded by this node
        fulfillment_received (list): total fulfillment received by this node
        demand_received (list): total demand received by this node (demand at this node)
        demand_fulfilled (list): total demand fulfilled by this node (demand that was served by this node)
        orders_shortage (list): total shortage of products at this node
        backorder (list): total backorders at this node
        inventory_level (float): current inventory level at this node
        inventory_waste (float): inventory waste at this node
        inventory_carry_cost (float): inventory carrying cost at this node
        inventory_spend_cost (float): inventory replenishment cost at this node
        transportation_cost (float): transportation cost at this node
        node_cost (float): total cost at this node
        revenue (float): revenue generated by this node
        profit (float): profit generated by this node (revenue - total cost)

    Functions:
        __init__: initializes the statistics object
        reset: resets the statistics to their initial values
        update_stats: updates the statistics with the given keyword arguments
        update_stats_periodically: updates the statistics periodically
...
class InventoryNode(Node):
    ...
    def process_order(self, supplier, reorder_quantity):
        ...
        if(supplier.source.node_status == "active"):
            self.inventory.on_hand += reorder_quantity # update the on-hand inventory level
            ...
            event, man_date_ls = supplier.source.inventory.get(reorder_quantity) # get reorder_quantity from supplier's inventory
            yield event
...
class Demand(Node):
    ...
    def wait_for_order(self,customer_id,order_quantity):
        ...
        if order_quantity > 0: # if the order quantity > 0, it means the order was not fulfilled

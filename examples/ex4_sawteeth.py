...
for node_id, node in supplynet["nodes"].items():
    if(node.node_type != "infinite_supplier"):
        ...
        axs[i].axhline(y=node.replenishment_policy.params['s'], color='r', linestyle='--',label=f"s = {node.replenishment_policy.params['s']}")  
        i += 1
fig.legend()
plt.xlabel('Time')
